...
Future<void> showAddSleepSheet(BuildContext context, WidgetRef ref) =>
    _sheet(context, _AddSleepForm(ref: ref));

class _AddSleepForm extends StatefulWidget {
  const _AddSleepForm({required this.ref});
  final WidgetRef ref;
  @override
...
class _AddSleepFormState extends State<_AddSleepForm> {
  ...
  Future<void> _submit() async {
    ...
    // Coucher = hier soir, réveil = ce matin (référence de nuit).
    ...
    await widget.ref.read(healthRepositoryProvider).addSleep(
          bedTime: bed,
          wakeTime: wake,
          quality: _quality,
...
Future<void> showAddPainSheet(BuildContext context, WidgetRef ref) =>
    _sheet(context, _AddPainForm(ref: ref));

class _AddPainForm extends StatefulWidget {
  const _AddPainForm({required this.ref});
  final WidgetRef ref;
  @override
...
class _AddPainFormState extends State<_AddPainForm> {
  ...
  Future<void> _submit() async {
    await widget.ref.read(healthRepositoryProvider).addPain(
          at: _at,
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Douleur', style: Theme.of(context).textTheme.titleLarge),
          const SizedBox(height: 16),
          TextField(
